...
import org.elasticsearch.dic.DictSegment;
import org.elasticsearch.dic.Dictionary;
import org.elasticsearch.dic.Hit;
import org.elasticsearch.utils.OptionPath;
...
public class ExtendFilter extends TokenFilter {
    ...
    public static final boolean DEFAULT_USE_FIRST_POSITION = true;
    // 在高亮的时候使用 offset
    public static final boolean DEFAULT_SHOW_OFFSET = true;
...
//            return o1.position != o2.position ? Integer.compare(o1.position, o2.position) : Integer.compare(o2.startOffset, o1.startOffset);
            ...
            } else if ((o1.endOffset-o1.startOffset) != (o2.endOffset-o2.startOffset)){
    ...
    public final boolean incrementToken() throws IOException {
        if (tokenBodies == null && input.incrementToken()) {
            ...
            do {
                ...
                // TODO useFirstPos=false 入库的特殊处理方式(不支持 offset 和 term_vector 存储方式)，否则就要改变 lucene源码。
            ...
            extendTerms(tokenBodies, indexMode, useFirstPos);
        }
    ...
    private void extendTerms(List<TokenBody> tokenBodies, boolean indexMode, boolean useFirstPos) {
        ...
        for (int beginI = 0; beginI < tokenBodies.size(); beginI++) {
            ...
            // 处理当前char, 但要考虑向后扩展
            List<Integer> endList = getCurrentEndList(tokenBodies, beginI);
...
//            }
     ...
     * @return
     */
    private List<Integer> getCurrentEndList(List<TokenBody> tokenBodies, int begin) {
